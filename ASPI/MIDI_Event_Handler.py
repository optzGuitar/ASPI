...
def set_tempo_handler(event, tracker):
    tracker.do_timestep(event.time)
    return f"tempo({tracker.track},{tracker.position},{event.tempo})."


def control_change_handler(event, tracker):
    tracker.do_timestep(event.time)
